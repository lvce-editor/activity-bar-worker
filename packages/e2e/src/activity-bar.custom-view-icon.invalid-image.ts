...
export const test: Test = async ({ Command, expect, Extension, Locator }) => {
  ...
  const iconUrl = new URL('src/invalid.png', toExtensionBaseUrl(uri)).href
  const hashInput = `test.views.customIconInvalidImage\n${iconUrl}`
  const customIconClass = `MaskIconCustomView${hashString(hashInput)}`
  const item = Locator('.ActivityBarItem[title="Custom Invalid Image Icon"]')
