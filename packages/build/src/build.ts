...
const packageJson = await readJson(join(root, 'packages', 'activity-bar-worker', 'package.json'))
...
packageJson.main = 'dist/activityBarWorkerMain.js'

await writeJson(join(dist, 'package.json'), packageJson)

await cp(join(root, 'README.md'), join(dist, 'README.md'))
await cp(join(root, 'LICENSE'), join(dist, 'LICENSE'))
await cp(join(root, 'packages', 'activity-bar-worker', 'settings.json'), join(dist, 'dist', 'settings.json'))
