...
const handleClickSettings = async (state: ActivityBarState, x: number, y: number, viewletId: string) => {
  await ContextMenu.show(x, y, MenuEntryId.Settings)
  return state
}

const handleClickAdditionalViews = async (state: ActivityBarState, x: number, y: number, viewletId: string) => {
  await ContextMenu.show(x, y, MenuEntryId.ActivityBarAdditionalViews)
  return state
}

const handleClickOther = async (state: ActivityBarState, x: number, y: number, viewletId: string) => {
...
export const handleClick = async (state: ActivityBarState, button: number, index: number, x: number, y: number): Promise<any> => {
